...
| 编号 | 内容 |
|------|------|
| P0-01～04 | 范围、前后端分离、错误语义、强类型报告 |
| P0-05 | 否定句/数值规则，禁止凑风险 |
| P0-06 | 事实分层与证据校验 |
| P0-07 | H5 四页主流程 |
| P0-08 | 端到端测试、金标与 `make test` |
...
会跑 API / 金标 / 否定句 / P0-05～08，并构建 H5。固定演示包见 `tests/fixtures/demo/mock_smoke_pack.json`（`demo_error` 可模拟超时与非法 JSON，不依赖真实模型）。
